...
__export(components_exports, {
  ...
  save: () => save3,
  search: () => search,
  searchByGeoWhereClause: () => searchByGeoWhereClause,
...
function load4(sharedInternalDocumentStore, raw) {
  ...
  const sorts = Object.keys(rawDocument.sorts).reduce((acc, prop) => {
    ...
    acc[prop] = {
      docs: new Map(Object.entries(docs).map(([k2, v2]) => [+k2, v2])),
      orderedDocsToRemove: /* @__PURE__ */ new Map(),
      orderedDocs,
      type
    };
...
// node_modules/@orama/orama/dist/browser/methods/pinning.js
...
async function removeMultipleAsync(orama, ids, batchSize, language, skipHooks) {
  ...
  await new Promise((resolve, reject) => {
    ...
    async function _removeMultiple() {
      ...
      for (const doc of batch) {
        try {
          if (await remove4(orama, doc, language, skipHooks)) {
            result++;
          }
        } catch (err) {
          reject(err);
...
function removeMultipleSync(orama, ids, batchSize, language, skipHooks) {
  ...
  function _removeMultipleSync() {
    ...
    if (!batch.length)
      return;
    for (const doc of batch) {
      if (remove4(orama, doc, language, skipHooks)) {
        result++;
      }
    }
...
function upsertMultiple(orama, docs, batchSize, language, skipHooks) {
  const asyncNeeded = isAsyncFunction(orama.afterInsert) || isAsyncFunction(orama.beforeInsert) || isAsyncFunction(orama.afterRemove) || isAsyncFunction(orama.beforeRemove) || isAsyncFunction(orama.beforeUpdate) || isAsyncFunction(orama.afterUpdate) || isAsyncFunction(orama.beforeUpsert) || isAsyncFunction(orama.afterUpsert) || isAsyncFunction(orama.beforeUpsertMultiple) || isAsyncFunction(orama.afterUpsertMultiple) || isAsyncFunction(orama.beforeInsertMultiple) || isAsyncFunction(orama.afterInsertMultiple) || isAsyncFunction(orama.beforeUpdateMultiple) || isAsyncFunction(orama.afterUpdateMultiple) || isAsyncFunction(orama.beforeRemoveMultiple) || isAsyncFunction(orama.afterRemoveMultiple) || isAsyncFunction(orama.index.beforeInsert) || isAsyncFunction(orama.index.insert) || isAsyncFunction(orama.index.afterInsert);
  if (asyncNeeded) {
...
var ORAMA_SECURE_PROXY_PLUGIN_NAME = "orama-secure-proxy";
var AnswerSession = class {
  ...
  async *fetchAnswer(params) {
    ...
    this.triggerStateChange();
    try {
      const sources = await search2(this.db, params);
      ...
      for await (const msg of this.proxy.chatStream({ model: this.chatModel, messages: this.messages })) {
        ...
        this.messages.findLast((msg2) => msg2.role === "assistant").content += msg;
        this.triggerStateChange();
      }
    } catch (err) {
  ...
  async init() {
    ...
    async function getPlugin() {
      return await self2.db.plugins.find((plugin2) => plugin2.name === ORAMA_SECURE_PROXY_PLUGIN_NAME);
    }
...
var kInsertions = /* @__PURE__ */ Symbol("orama.insertions");
var kRemovals = /* @__PURE__ */ Symbol("orama.removals");
...
export {
  ...
  getByID,
  count2 as count,
  components_exports2 as components_exports,
  insert3 as insert,
  insertMultiple,
  insertPin,
  updatePin,
...
//# sourceMappingURL=chunk-366RCIOA.js.map
